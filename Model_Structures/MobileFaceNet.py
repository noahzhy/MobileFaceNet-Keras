...
from tensorflow.python.keras.layers import (
    Input, Conv2D, BatchNormalization, ReLU, PReLU, SeparableConv2D, 
    DepthwiseConv2D, add, Flatten, Dense, Dropout
)
...
def bottleneck(inputs, filters, kernel, t, s, r=False):
    ...
    Z1 = DepthwiseConv2D(kernel, strides=s, padding='same',
                         depth_multiplier=1, use_bias=False)(Z1)
...
def linear_GD_conv_block(inputs, kernel_size, strides):
    channel_axis = 1 if K.image_data_format() == 'channels_first' else -1
    Z = DepthwiseConv2D(kernel_size, strides=strides, padding='valid',
                        depth_multiplier=1, use_bias=False)(inputs)
    Z = BatchNormalization(axis=channel_axis)(Z)
    return Z
...
def mobile_face_net_train(num_labels, loss='arcface'):
    ...
    M = inverted_residual_block(
        M, 128, 3, t=4, strides=2, n=1)  # (14, 14, 128)
    M = inverted_residual_block(
        M, 128, 3, t=2, strides=1, n=6)  # (14, 14, 128)
    ...
    M = Flatten()(M)
    M = Dense(128, activation=None, use_bias=False,
              kernel_initializer='glorot_normal')(M)
